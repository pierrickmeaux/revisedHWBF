...
\begin{Prop}\label{prop:wwd1}
	...
	\begin{itemize}
		\item (\eg{}~\cite{DCC:DalMaiSar06}, Theorem $3$) The majority function $\mathsf{Maj}_n$ introduced in Definition~\ref{def:maj} satisfies:
		\[
		...
		\NL(h)=2^{n-1}-2\binom{n-2}{\frac{n-2}{2}}.
		\]	
		\item (\cite{DAM:WCST14}, Theorem $1$) The \hwbf{} $h$ introduced in Definition~\ref{def:hwbf} satisfies $\degg(h)=n-1$ for all $n\geq 4$.
	\end{itemize}
\end{Prop}
...
\begin{definition}[Krawtchouk polynomials]\label{def:Kraw}
    ...
    \[
        \displaystyle \kraw{k}{x}{n}=\sum_{j=0}^{k} (-1)^j \binom{x}{j} \binom{n-x}{k-j}.
    \]
    Alternatively, the Krawtchouk polynomials can be characterized as the coefficients in the generating function for $(1+z)^{n-x} (1-z)^x$ in the variable $z$:
    \[
        (1+z)^{n-x} (1-z)^x=\sum_{k=0}^\infty \kraw{k}{x}{n} z^k.
    \]
\end{definition} 
...
\[f(x)=\sum_{i=1}^{n/2}(x_{2i-1}+1)x_{2i}+ \sum_{k=1}^n x_k\phikn{k}{n}(x).\]
\end{definition}
...
Since $f$ is the sum of a quadratic function and a weightwise affine function, it is a weightwise quadratic function. 
Using the formalism of Definition~\ref{def:wwdegd}, $f$ is the weightwise quadratic function defined by $f_0(x)=0$ and $f_k(x)=\sum_{i=1}^{n/2}(x_{2i-1}+1)x_{2i}+x_k$ for all $k\in[1,n]$. This form allows to derive the restricted Walsh transform of $f$, which can be a useful tool to study the balancedness and bound the nonlinearity of a function.
%Maybe add examples, such as OM24, M24

\begin{definition}
    For an even integer $n\geq 0$, we introduce the Boolean function $d_n\in\BN$ given by:
    \[
...
Let $n\geq 2$ be an even integer. Let $a\in\F_2^n$ and let $k\in[1,n]$. If we put $c=a+e_k+\sum_{i=1}^{n/2}e_{2i}$, then the restricted Walsh transform of the revisited \hwbf{} $f\in\BN$ satisfies:
\[
    \wtk{f}{k}(a)=\sum_{x \in \Ekn{k}{n}} (-1)^{d_n(x)+c\cdot x}=\wtk{d_n}{k}(c).
\]
\end{proposition}
...
\begin{align*}
    \wtk{f}{k}(a)&=\sum_{x \in \Ekn{k}{n}} (-1)^{f(x)+a \cdot x}\\
    &=\sum_{x \in \Ekn{k}{n}} (-1)^{\sum_{i=1}^{n/2}(x_{2i-1}+1)x_{2i}+x_k+a \cdot x}\\
    &=\sum_{x \in \Ekn{k}{n}} (-1)^{\sum_{i=1}^{n/2}x_{2i-1}x_{2i}+\left(x_k+\sum_{i=1}^{n/2}x_{2i}+a \cdot x\right)}\\
    &=\sum_{x \in \Ekn{k}{n}} (-1)^{\sum_{i=1}^{n/2}x_{2i-1}x_{2i}+c\cdot x}.\qedhere
\end{align*}
\end{proof}
...
Accordingly, we can study the restricted Walsh transform of the \hwbf{} by analyzing the one of $d_n$. We introduce the following notations:

\begin{definition}
Let $n\geq 0$ be an even integer and $a\in\F_2^n$ a vector. Then for every integer $k$, we define $\Dnka{n}{k}{a}=0$ if $n\not\in[0,n]$, and otherwise:
\[
...
\begin{itemize}
    ...
    \item If $n\geq 2$, then for $b=(a_1,\dots,a_{n-2})$, we have:
...
\begin{proof}
    ...
    &=\sum_{y\in \Ekn{k}{n-2}} (-1)^{d_{n-2}(y) +b y}+
    \sum_{y\in \Ekn{k-1}{n-2}} (-1)^{d_{n-2}(y) +b y + a_{n-1}}\\
    &\qquad+\sum_{y\in \Ekn{k-1}{n-2}} (-1)^{d_{n-2}(y) +b y + a_{n}}
    +\sum_{y\in \Ekn{k-2}{n-2}} (-1)^{d_{n-2}(y) +b y +1 + a_{n-1}+ a_{n}}\\
...
\begin{proposition}\label{prop:WT}
    Let $n\geq 0$ be even, and let $a\in\F_2^n$. Put $b=a+\sum_{i=1}^{n/2}e_{2i}$. Then the Walsh transform of the revisited \hwbf{} $f\in\BN$ at $a$ is given by:
    \[ \wt{f}(a)=1+\sum_{k=1}^n \Dnka{n}{k}{b+e_k}.\]
\end{proposition}

\begin{proof}
For every $k\in[1,n]$, define $c_k=a+e_k+\sum_{i=1}^{n/2}e_{2i}=b+e_k$. Then, using Proposition~\ref{prop:restrWT}, we get:
\[
    \wt{f}(a)=1+\sum_{k=1}^n\wtk fk(a)=1+\sum_{k=1}^n\wtk{d_n}k(c_k)=1+\sum_{k=1}^n\Dnka nk{c_k}=1+\sum_{k=1}^n\Dnka nk{b+e_k}.\qedhere
\]
\end{proof}

\subsection{Balancedness of $f$}\label{sec:balancedness}

In this part, we determine for which values of $n$ the revisited \hwbf{} $f\in\BN$ is balanced by using the expression of its Walsh transform in terms of the $\Dnka{n}{k}{a}$ together with the recursive relation of these quantities.

\begin{theorem}\label{th:bal}
    For an even integer $n\geq 2$, the following holds true for the Walsh transform of the revisited \hwbf{} $f\in\BN$ at $0_n$:
	\[\wt{f}(0_n)= \left \{
	\begin{array}{l l}
	0 & \text{ if } n/2 \text{ is even, } \\
	-2 \binom{n/2-1}{(n-2)/4} & \text{ if }  n/2 \text{ is odd.}
	\end{array}\right.\]
	Accordingly, $f$ is balanced when $n \equiv 0\bmod 4$ and unbalanced when $n\equiv 2 \bmod 4$.
\end{theorem}
...
\begin{definition}\label{defi:P_a}
    Let $n\geq 2$ be an even integer. For $a\in\F_2^n$, denote by $p=p(a)$ the number of odd $i\geq 1$ for which $(a_i,a_{i+1})=(0,0)$, by $q=q(a)$ the number of odd $i\geq 1$ for which $(a_i,a_{i+1})=(1,1)$, and by $r=r(a)$ the number of odd $i\geq 1$ for which $(a_i,a_{i+1})\in\{(0,1),(1,0)\}$. We then have $p+q+r=n/2$. Consider further the integer polynomial $P_a(z)$ given by the following expression:
    \[
...
\pmnote{Show me screenshots proving that 2 and 4 are correct}
...
$\NL(f)\geq 2^{n-1} - \frac{1}{2} -2^{\frac{3n}{4} + \log n -1}$.
\end{Corollary}
...
\begin{proposition}\label{proposition:dtn}
	Let $t, n\in \N$ with $n$ even and $t\leq n/2$, the following holds on $d_{t,n}$ for all $k\in [0,n]$:
...
\begin{definition}\label{defi:P_a_t}
    Let $t, n\in \N$ with $n$ even and $t\leq n/2$. Given $a=(b,c)\in\F_2^n$ with $b\in\F_2^{2t}$, denote $n-2t-\w(c)$ and $\w(c)$ by $u$ and $v$, respectively. We  then introduce the following polynomial:
    \[
...
\begin{proposition}
    Let $t, n\in \N$ with $n$ even and $t\leq n/2$. Let $a\in\F_2^n$. The following holds true: 
    \[
...
\begin{theorem}\label{theorem:bound_D_nka_generalized}
    Let $t, n\in \N$ with $n$ even and $t\leq n/2$. 
...
In this section, we establish a tighter bound on the absolute value of the Walsh transform of the revisited \hwbf{} for values of $n$ up to $80$. 
...
\begin{proposition}[Algebraic degree of $f$]
...
\begin{proof}
	...
	Using Property~\ref{prop:wwd1} Item $3$, $h$ has degree $n-1\geq 3$, and the function $q$ has no monomial of degree higher than $2$, therefore $\degg(f)=\degg(h)=n+1$.
...
\begin{proof}
	...
	If $g (1+q)\ne 0$ then $\degg(g(1+q)) \leq \degg(g)+2$ and we obtain $\degg(g)\geq \AI(h) -2$. 
	Otherwise, $g (1+q)= 0$ implies that $g$ annihilates $1+q$ and therefore $0=g (f+\varepsilon + 1 +q)=g(h+ \varepsilon +1)$ that is $\degg(g)\geq \AI(h)$. 
	Summarizing, for any non null annihilator $g$ of $f$ or $f+1$ we get $\degg(g)\geq \AI(h)-2$, it allows to conclude $\AI(f)\geq \AI(h)-2$.
\end{proof}
...
\pmnote{add a small conclusion (summary of contributions), and elaborate on an opean question, for example on how the Cauchy estimate could be used for more families, other quadratic functions or more easily higher degree ones.}
