...
\begin{table}
	...
	\mycaption{Values of $\Dnka{2}{k}{a}$ for $n=2$.}\label{tab:Dnka}
\end{table}
...
\begin{proof}
	...
	\begin{align*}
		\wt{f}(a)&=\sum_{x \in \Ekn{k}{n}} (-1)^{f(x)+a\cdot x}\\
		...
		&=\Dnka{n}{k}{b+\pi(e_k)}.
	\end{align*}
	We conclude by applying $\wtk{f}{k}(a)=1+\sum_{k=1}^n\wtk{f}{k}(a)$.
\end{proof}
...
\begin{proof}
	Applying Remark~\ref{rmk:dependence_dnka} and that $\pi^{-1}(0_n)=0_n$,
	Proposition~\ref{prop:WT} gives:
	\begin{align*}
	...
	Similarly, we get:
	\[
		\Dnka{n}{k}{0,0,0,1,\ldots,0,1}=\binom{n/2-1}{k/2} +2 \binom{n/2-1}{(k-1)/2}-\binom{n/2-1}{k/2-1}.\\
	\]
	...
	\end{align*}
	The announced result follows by applying that $\binom{m}{k}=\binom{m}{m-k}$ for all integers $m\geq 0$ and $k$.
\end{proof}
...
\begin{table}
	\small
    	...
    	\mycaption{Values of $\Dnka{n}{k}{a}$ for small values of $k$.}\label{tab:Dnka_small_k}
    \end{table}
...
\begin{remark}
    ...
    \begin{figure}
        ...
        \mycaption{Cauchy estimate in blue vs.\ binomial coefficients in red.}
...
We consider now the complexity involved in computing the values of $\mathsf{D}_{n,k}^{p,q,r}$ for all $k\in[1,n]$ and all triplets $(p,q,r)$ satisfying $p+q+r=n/2$. We claim that this computation has a complexity of $\mathcal{O}(n^3 \log n)$. Towards this, it is sufficient to expand $(-z^2 + 2z + 1)^p \cdot (-z^2 - 2z + 1)^q \cdot (z^2 + 1)^r$ for all triplets $(p,q,r)$ such that $p+q+r=n/2$; this is due to Proposition \ref{proposition:generating_fct}. To achieve this, we first precompute the expanded polynomial $(-z^2 + 2z + 1)^p$ for each $p\in[1,n/2]$. This can be done recursively using the formula $(-z^2 + 2z + 1)^{p+1} = (-z^2 + 2z + 1) \cdot (-z^2 + 2z + 1)^p$. For each $p$, this requires $\mathcal O(n)$ arithmetic computations. Performing this for every $p$ results in a total complexity of $\mathcal{O}(n^2)$. The same approach can be used to expand the polynomials $(-z^2 - 2z + 1)^q$ and $(z^2 + 1)^r$, resulting in an overall complexity of $\mathcal{O}(n^2)$ for a fixed triplet $(p,q,r)$. Finally, for each such triplet $(p,q,r)$, we multiply the three expanded polynomials $(-z^2 + 2z + 1)^p$, $(-z^2 - 2z + 1)^q$ and $(z^2 + 1)^r$. These two multiplications are performed in $\mathcal{O}(n \log n)$ operations by using the fast Fourier transform. Given that there are $\mathcal{O}(n^2)$ triplets $(p,q,r)$ to consider, precomputing the values $\mathsf{D}_{n,k}^{p,q,r}$ for all $k\in[1,n]$ and all triplets $(p,q,r)$ has a complexity of $\mathcal{O}(n^3 \log n)$, making the overall complexity of the procedure also $\mathcal{O}(n^3 \log n)$.
...
\bigskip
In Table~\ref{table:max_walsh_vs_bound}, we compare the exact values of $\max_{a\in\mathbb F_2^n}|\wt f(a)|$ for small values of $n$ (calculated using \textsf{SageMath}) to the bound $B_n$ obtained through the above method and the bound from Corollary~\ref{cor:bound_walsh_cauchy}. Additionally, Table~\ref{table:walsh_bounds} provides the values of the bound $B_n$ for all even values of $n\in[1,80]$. The method described above offers a significantly tighter bound compared to Corollary~\ref{cor:bound_walsh_cauchy}. Its polynomial complexity makes it possible to extend the analysis well beyond the limitations imposed by a full computation of the Walsh spectrum, which requires a complexity of $\mathcal O(n2^n)$. We provide a \textsf{SageMath} implementation to obtain the bound $B_n$: \url{https://github.com/se-tim/Revisited-HWBF.git}.
...
\begin{table}
	...
	\mycaption{The actual $\max_{a\in\F_2^n}|\wt f(a)|$ compared to the bound $B_n$ and the bound from Corollary~\ref{cor:bound_walsh_cauchy}.}
...
In this part, we compare the nonlinearity of the revisited \hwbf{} $f\in\BN$ to other weightwise quadratic functions considered for similar use-cases, such as the majority function, the \hwbf{} and the cyclic weightwise functions studied in~\cite{DAM:MeaOza24}. 
...
\begin{figure}
	\centering
	\includegraphics[width=10.5cm]{comparison_walsh_bound.pdf}
	...
	\mycaption{
		...
		Bounds on the maxima of absolute values of the Walsh spectrum for various functions, shown on a logarithmic scale for even values of $n$.
		The violet and orange curves, the orange one mostly covering the violet one, represent bounds for cyclic weightwise linear functions and a cyclic weightwise quadratic function from~\cite{DAM:MeaOza24}, respectively. The blue curve shows bounds for the majority function and the \hwbf{}. The theoretical bound for the revisited \hwbf{} (from Corollary~\ref{cor:bound_walsh_cauchy}) is in red, and the experimental bound $B_n$ (Section~\ref{sec:expwt}) is in green.}
	\label{fig:walsh_bound_comparison}
\end{figure}

The revisited \hwbf{} demonstrates significantly better performance compared to the majority function and the original \hwbf{}. Its theoretical bound reveals an asymptotic slope of $3/4$ on a logarithmic scale, contrasting with the slope of $1$ for the majority function and the \hwbf{}. Notably, the green curve corresponding to the bound of Section~{sec:expwt} appears to share this asymptotic slope of $3/4$, supporting the effectiveness of Corollary~\ref{cor:bound_walsh_cauchy} and the stronger Theorem~\ref{theorem:bound_D_nka_generalized} in capturing asymptotic behavior. In contrast, the bounds proven in~\cite{DAM:MeaOza24}, represented by the violet and orange curves, exhibit the less favorable asymptotic slope of $1$.
...
\iffalse
 \begin{table}[h!]
	 ...
	 \mycaption{Characteristics of Boolean Functions for Different $n$ Values}
	 \end{table}
\fi
