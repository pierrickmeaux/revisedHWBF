...
\pmnote{add}
\begin{definition}[Majority functions]\label{def:maj}
\end{definition}
...
\pmnote{complete the following prop. Choose between nonlinearity or $\max_{a\in\mathbb F_2^n}|\wt f(a)|$}

\begin{Prop}[Cryptographic properties of functions from $\WD{2}{n}$ \label{prop:wwd1}]
Let $n\in \N^*$ even, the following hold for functions in $\WD{2}{n}$. 
\begin{itemize}
	\item For the majority function
	
	$\max_{a\in\mathbb F_2^n}|\wt f(a)|$ (adapt from Theorem 3 \cite{DCC:DalMaiSar06})
	\item value for HWBF. (from~\cite{DAM:WCST14}))
...
\pmnote{I would recall between parentheses somewhere that we denote by $|.|$ the complex norm here, which corresponds to the absolute value on $\R$

verify if $w$ and $z$ are coherent.
}
...
When a specific value of $t$ is given in terms of $n$, the proof strategy for Theorem~\ref{theorem:bound_D_nka_generalized} can be applied more precisely. For instance, we consider the case where $t = n/3$:
...
\begin{proof}
   ...
   To find a bound for $|P_{a,t}(z)|$ on $\dis$ that applies to all $a\in\F_2^n$, we focus only on values of $a$ which are candidates for maximizing $|P_{a,t}(z)|$. 
   Specifically, we consider cases where only one of $(p,q,r)$ is non-zero, and for which only and only one of $(u,v)$ is non-zero. 
   
\pmnote{What is the argument: "maximum implies only one is non-zero" }
   ...
   This leaves us with the following cases to consider for $(p,q,r;u,v)$:
    ...
    \pmnote{@Tim, add intermediate computation in the following:}
    \[
        |P_{a,t}(z)|^2=[(6-2\cos2\alpha)(2+2\cos\alpha)]^t.
    \]
    \pmnote{@Tim, for the following, by calculus, you mean using some software? In this case we can add a footnote saying in practice we did it with xxx.
    	
    Can you give a plot of $(\alpha,(6-2\cos2\alpha)(2+2\cos\alpha))$ and add the line corresponding to $y=9/4$?
    }

    Through calculus, one finds that $|(6-2\cos2\alpha)(2+2\cos\alpha)|<2^{9/2}$, and hence that $|P_{a,t}(z)|<2^{9t/4}=2^{3n/4}$. The same upper bound works for the remaining two candidates for $(p,q,r;u,v)$, and we can therefore conclude  $|\wtk{d_{n/3,n}}{k}(a)|\leq 2^{3n/4}$, and $|\wt{d_{n/3,n}}(a)|\leq 1+n\cdot 2^{3n/4}$.
\end{proof}


\pmnote{for the remark, add how they are obtained (from a script in xxx). 
We can give the scripts in supplementary material when submitting, of add the link of a github if you agree to have them public.
...
\begin{remark}
    ...
    In Figure~\ref{fig:walsh_bounds_generalization}, we provide bounds of the form $\max_{a\in\F_2^n}|\wt{d_{\alpha n/40,n}}(a)|\leq 1+n\cdot 2^{\beta n}$, where $\alpha\in[0,20]$ is represented on the $x$-axis and $\beta$ is represented on the $y$-axis. 
\end{remark}

\begin{figure}
	\centering
	\includegraphics[width=9cm]{walsh_bounds_generalization.pdf}
	\caption{Improvements on Corollary~\ref{cor:gen}. For each point $(\alpha,\beta)$, we have $\max_{a\in\F_2^n}|\wt{d_{\alpha n/40,n}}(a)|\leq 1+n\cdot 2^{\beta n}$.}
...
\begin{table}
	...
	\end{tabular}
	\caption{The actual $\max_{a\in\F_2^n}|W_f(a)|$ compared to the bound $B_n$}
