...
\pmnote{\color{blue} Tim: Check below}
\begin{definition}[Majority function]\label{def:maj}
    The \emph{majority function} in $n$ variables is the Boolean function $f\in\BN$ defined as:
    \[
        f(x)=0\iff\w(x)\leq\frac n2.
    \]
\end{definition}
...
\begin{Prop}[Cryptographic properties of functions from $\WD{2}{n}$, \cite{DCC:DalMaiSar06,DAM:WCST14,DAM:MeaOza24,DAM:MeaOza24}\label{prop:wwd1}]
...
\pmnote{I would recall between parentheses somewhere that we denote by $|.|$ the complex norm here, which corresponds to the absolute value on $\R${ \color{blue} Tim :OK, check above remark.}
...
\pmnote{What is the argument: "maximum implies only one is non-zero" \color{blue} Tim: OK, check above lemma}
   ...
   This leaves us with the following cases to consider for $(p,q,r;u,v)$:
    ...
    \]
    Writing $z=\cos\alpha+\mathrm i\sin\alpha$ and $z=\cos2\alpha+\mathrm i\sin2\alpha$, we have:
    \begin{align*}
        |-z^2+2z+1|^2&=|-z^2+2z+1|^2\\
        &=|-\cos2\alpha-\mathrm i\sin2\alpha+2\cos\alpha+2\mathrm i\sin\alpha+1|^2\\
        &=(-\cos2\alpha+2\cos\alpha+1)^2+(-\sin2\alpha+2\sin\alpha)^2\\
        &=6-2\cos(2\alpha);
    \end{align*}
    the last equality can be obtained by expanding and applying classical trigonometric identities, or by making use of a computer algebra system such as \textsc{SageMath}. We similarly obtain that $|1+z|^2=2+2\cos\alpha$. Combining both, we get:
    
    \pmnote{@Tim, add intermediate computation in the following: \color{blue} Tim: Done}
    ...
    Since $|(6-2\cos2\alpha)(2+2\cos\alpha)|<2^{9/2}$ as shown in Figure~\ref{fig:trigo_bound} (which can also be formally checked using classical tricks from calculus), we obtain that $|P_{a,t}(z)|<2^{9t/4}=2^{3n/4}$. The same upper bound works for the remaining two candidates for $(p,q,r;u,v)$, and we can therefore conclude  $|\wtk{d_{n/3,n}}{k}(a)|\leq 2^{3n/4}$, and $|\wt{d_{n/3,n}}(a)|\leq 1+n\cdot 2^{3n/4}$.
\end{proof}

\begin{figure}
	\centering
	\includegraphics[width=9cm]{trigo_bound.pdf}
	\caption{The function $\alpha\mapsto(6-2\cos2\alpha)(2+2\cos\alpha)$ in blue, and the line $y=2^{9/2}$ in red.}
