...
%\pmnote{For example, the filter functions leading to the best current performances for FiLIP in HHE~\cite{CCS:CDPP22,CIC:MeaParPei24,eprint:AGHM24} are xor-threshold functions, the direct sum of a $k$-variable linear function and an $m$-variable function giving $1$ if and only if the input has Hamming weight at least a threshold $d$ (which generalizes the majority function). Using Proposition $7$ in~\cite{IEEE:CarMea21}, the nonlinearity of a xor-threshold function is at most:
%\[2^{n-1}-2^k \cdot M \text{ where }
%M=\left\{\begin{array}{ll}
%\binom{m-1}{\frac{m-1}{2}}&\text{ if }m\equiv 1\bmod 2,\\
%\frac{1}{2} \binom{m}{\frac{m}{2}}&\text{ if }m\equiv 0\bmod 2.\\
%\end{array}\right.
%\]
%The nonlinearity of this whole family of functions is similar to the one of the majority, hence the revisited HWBF provides a better nonlinearity that currently used filters for FiLIP for the same sizes, allowing to thwart correlation-like attacks with functions in less variables.}
For example, the best current \textsf{FiLIP} filters in HHE~\cite{CCS:CDPP22,CIC:MeaParPei24,eprint:AGHM24} are xor-threshold functions—the sum of a $k$-variable linear function and an $m$-variable threshold function (generalizing majority).  
By Proposition $7$ in~\cite{IEEE:CarMea21}, their nonlinearity is at most:
\[
2^{n-1} - 2^k \cdot M, \quad \text{where } M = \begin{cases}
\binom{m-1}{\frac{m-1}{2}} & \text{if } m \text{ odd}, \\
\frac{1}{2} \binom{m}{\frac{m}{2}} & \text{if } m \text{ even}.
\end{cases}
\]
This is similar to majority, whereas the revisited HWBF achieves higher nonlinearity for the same size, offering better resistance to correlation-like attacks with fewer variables.
\begin{table}
	\scriptsize
	\centering
