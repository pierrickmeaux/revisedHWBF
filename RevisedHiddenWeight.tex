...
In this paper we mainly consider the slice indicator functions.
...
\pmnote{adapt usefull parts:



In~\cite{DAM:GinMea22} the authors study the relationship between weightwise perfectly balanced functions (functions balanced on all the slices, see for example~\cite{TOSC:CarMeaRot17,DCC:LiuMes19,CC:TangLiu19,BFA:MesSuLi21,ACI:ManKuLar22,SSRN:ZLCQZ23,ASIACCS:YCL+23,LATIN:GinMea23}) and weightwise affine functions \ie weightwise degree-$1$ functions. 
In fact weightwise degree-$1$ and degree-$0$ functions have been studied for their cryptographic properties in many works, without the same formalism. 
The weightwise constant functions ($\WD{0}{n}$) are the symmetric functions that have been extensively studied (\eg \cite{IEEE:Carlet04,IEEE:CanVid05,INDO:BraPre05,DM:SarMai07,IEEE:CheLu11,A2C:CarMea19,Latin:Meaux19,CCDS:Meaux21,IEEE:CarMea21}). 
Thereafter, the hidden weight bit function introduced in~\cite{IEEE:Bryant91} is a weightwise degree-$1$ function, the one obtained by fixing $f_0=0$ and $f_k=x_k$ for $k \in [n]$. 
The cryptographic properties of this function have been studied in~\cite{DAM:WCST14}, showing good algebraic properties for this function. 
In~\cite{TOSC:CarMeaRot17}, the bent functions in Propositions $1$ and $2$ are weightwise affine.}
...
\begin{definition}[Weightwise degree-$d$ functions]\label{def:WdegD}
	...
	%	Equivalently, $f$ is weightwise affine if and only if for each $k\in [0,n]$ $\NLk{k}(f)=0$.
	
	The set of weightwise degree-$d$ functions is denoted by $\WD{d}{n}$.
	
	
	
	%	$f=\sum_{k=0}^n f_k \phikn{k}{n}$ is called weightwise affine if and only if for each $k\in [0,n]$ $f_k$ coincide with an affine function over $\Ekn{k}{n}$.
	
\end{definition}

\pmnote{add: properties of maj function, HWPF, and some WWL and WWQ}

\pmnote{add the nonlinearity of Dickson's function}
