...
\pmnote{complete the following prop, + references}

\begin{property}[Cryptographic properties of functions from $\WD{1}{n}$ \label{prop:wwd1}]
...
\begin{remark}\label{remark:D_nka_differentiation}
    We can also use Proposition \ref{proposition:generating_fct} to obtain $\Dnka nka$ through differentiation:
    \[
...
\subsection{Bounding the Walsh transform of $f$ using the Cauchy estimate}

Recall the Cauchy estimate on holomorphic functions:

\begin{Prop}[\cite{rudin1987}, Theorem 10.26]\label{property:cauchy_estimate}
...
\begin{proof}
    Recall from Remark \ref{remark:D_nka_differentiation} that $k!\cdot\Dnka nka=\frac{\dd^k}{\dd^k z}P_a(z)\vert_{z=0}$. Choosing $f=P_a$, $w=0$ and $R=1$ in Property \ref{property:cauchy_estimate} then gives:
    \[
        |\Dnka nka|=\frac{1}{k!}\left|\frac{\dd^k}{\dd^k z}P_a(z)\vert_{z=0}\right|\leq M,
    \]
    where $M$ is the maximum of $|P_a(z)|$ on the circle $|z|=1$. We claim, and prove below, that the maximum of $\left|-z^2+2z+1\right|$ on $|z|=1$ is $2\sqrt 2$. This implies that the maximum of $\left|-z^2-2z+1\right|$ on $|z|=1$ is $2\sqrt 2$ as well. Since the maximum of $\left|z^2+1\right|$ on $|z|=1$ is $2<2\sqrt 2$, it follows that $|\Dnka nka|\leq M\leq (2\sqrt 2)^{p+q}2^r\leq (2\sqrt 2)^{n/2}$.

    It remains to prove that the maximum of $\left|-z^2+2z+1\right|^2$ on $|z|=1$ equals $8$. Using that $|w|^2=w\overline w$ for every complex number $w$, we obtain for $|z|=1$:
    \[
    ...
    \]
    where $\mathrm{Re}\left(z^2\right)$ denotes the real part of $z^2$. Since the minimum of $\mathrm{Re}\left(z^2\right)$ on $|z|=1$ is $-1$, we conclude that the maximum of $\left|-z^2+2z+1\right|^2$ on $|z|=1$ is $6-2(-1)=8$.
\end{proof}
...
\begin{proof}
    We have previously established that $\wt{f}(a)=1+\sum_{k=1}^n\Dnka{n}{k}{b+\pi(e_k)}$. Then, applying Proposition \ref{theorem:bound_D_nka} yields:
    \[
...
When one is given an explicit value for $t$ in terms of $n$, one can be more careful when employing the proof strategy of Theorem~\ref{theorem:bound_D_nka_generalized}. Let us for instance consider the case $t=n/3$:

\begin{proposition}\label{proposition:walsh_t_over_3}
    Let $n\in\N$ be divisible by $6$. Then for every $a\in\F_2^n$ and every $k\in[0,n]$, it holds that:
    \[
        |\wtk{d_{n/3,n}}{k}(a)|\leq2^{3n/4},\quad|\wt{d_{n/3,n}}(a)|\leq 1+n\cdot 2^{3n/4}.
    \]
\end{proposition}

\begin{proof}
    As before, we want to bound the maximum of $|P_{a,t}(z)|$ on $|z|=1$ for $a\in\F_2^n$, and this will then be an upper bound on $|\wtk{d_{t,n}}{k}(a)|$ for every $k\in[0,n]$. Write $a=(b,c)$ with $b\in\F_2^{2t}$ and let $(p,q,r)=(p(b),q(b),r(b))$, and further consider $u=n-2t-\w(c)$ and $v=\w(c)$ as in Definition~\ref{defi:P_a_t}. Since we want to find a bound of $|P_{a,t}(z)|$ on $|z|=1$ which works for all $a\in\F_2^n$, it makes sense to only consider those $a$ which are actually candidates for maximizing $|P_{a,t}(z)|$, namely those for which only one among $(p,q,r)$ is non-zero, and for which only one among $(u,v)$ is non-zero. This leaves us with the following cases to consider for $(p,q,r;u,v)$:
    \begin{gather*}
        (t,0,0;n-2t,0),\quad(0,t,0;n-2t,0),\quad(0,0,t;n-2t,0),\\
         (t,0,0;0,n-2t),\quad(0,t,0;0,n-2t),\quad(0,0,t;0,n-2t).
    \end{gather*}
    Since swapping both $(p,q)$ and $(u,v)$ has the same effect as replacing $z$ by $-z$ in $P_{a,t}(z)$ and therefore does not change the maximum of $|P_{a,t}(z)|$ on $|z|=1$, we can further restrict ourselves to the following tuples for $(p,q,r;u,v)$:
    \[
        (t,0,0;n-2t,0),\quad(0,t,0;n-2t,0),\quad(0,0,t;n-2t,0).
    \]
    ...
    \]
    Writing $z=\cos\alpha+\mathrm i\sin\alpha$, we obtain:
    \[
        |P_{a,t}(z)|^2=[(6-2\cos2\alpha)(2+2\cos\alpha)]^t.
    \]
    Through calculus, one finds that $|(6-2\cos2\alpha)(2+2\cos\alpha)|<2^{9/2}$, and hence that $|P_{a,t}(z)|<2^{9t/4}=2^{3n/4}$. The same upper bound works for the remaining two candidates for $(p,q,r;u,v)$, and we can therefore conclude that $|\wtk{d_{n/3,n}}{k}(a)|\leq 2^{3n/4}$ and that $|\wt{d_{n/3,n}}(a)|\leq 1+n\cdot 2^{3n/4}$.
\end{proof}

\begin{remark}
    The proof of Proposition~\ref{proposition:walsh_t_over_3} can be redone for other values of $t$. In Figure~\ref{fig:walsh_bounds_generalization}, we provide bounds of the form $\max_{a\in\F_2^n}|\wt{d_{\alpha n/40,n}}(a)|\leq 1+n\cdot 2^{\beta n}$, where $\alpha\in[0,20]$ is represented on the $x$-axis and $\beta$ is represented on the $y$-axis. 
\end{remark}

\begin{figure}
	\centering
	\includegraphics[width=9cm]{walsh_bounds_generalization.pdf}
	\caption{For each point $(\alpha,\beta)$, we have $\max_{a\in\F_2^n}|\wt{d_{\alpha n/40,n}}(a)|\leq 1+n\cdot 2^{\beta n}$.}
