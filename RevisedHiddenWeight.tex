...
Throughout this paper, $n$ will always denote a positive integer. Further, our intervals will only contain integers, so that for instance $[0,n]=\{0,1,\dots,n\}$. The set of binary vectors of length $n$ will be denoted by $\F_2^n$. The entries of a binary vector $x\in\F_2^n$ will always be denoted by $x_1,\dots,x_n$. We denote the scalar product of two binary vectors $x,y\in\F_2^n$ by $x\cdot y=\sum_{i=1}^nx_iy_i\in\F_2$. The Hamming weight of a binary vector $x\in\F_2^n$ is denoted by $\w(x)=|\{i\in [1,n]:x_i=1\}|$.%Tim: Check
...
\begin{definition}[Boolean function, support]\label{def:bool_f}
...
\begin{definition}[Support, balancedness]\label{def:balancedness}
	Consider a Boolean function $f\in\BN$. Then the support of $f$ is defined as $\supp(f)=\{x\in\F_2^n:f(x)\neq 0\}$. The function $f$ is called balanced if it satisfies the equalities $|\supp(f)|=2^{n-1}=|\supp(f+1)|$. 
	
%	For $k\in [0,n]$ the function is said balanced on the slice $k$ if $||\suppk{k}(f)|-|\suppk{k}(f+1)| |\leq 1$. In particular when $|\Ekn{k}{n}|$ is even $|\suppk{k}(f)|=|\suppk{k}(f+1)|=|\Ekn{k}{n}|/2$.
\end{definition}
...
\bigskip
The algebraic degree of multiples of a Boolean function is a crucial quantity for various attacks on stream ciphers that utilize a Boolean function as a filter. %Tim: Check previous sentence
...
The Boolean symmetric functions in $n$ variables are those that are constant on each slice $\Ekn{k}{n}$ for $k\in [0,n]$. 
This class has been thoroughly studied in the context of cryptography, see \eg{} \cite{IEEE:Carlet04,IEEE:CanVid05,INDO:BraPre05,DM:SarMai07,IEEE:QFLW09,IEEE:CheLu11,Latin:Meaux19,CCDS:Meaux21,IEEE:CarMea21}.
...
\begin{definition}[Weightwise degree-\textit{d} functions (~\cite{DAM:MeaOza24} Definition 12)]\label{def:wwdegd}
...
\begin{Prop}[Properties of some functions from $\WD{1}{n}$\label{prop:wwd1}]
...
We use Krawtchouk polynomials and some of their properties to prove one of our main results. We give the necessary preliminaries here and refer to \eg{}~\cite{book:MacSlo78} for more details.
...
\begin{definition}[Krawtchouk Polynomials]\label{def:Kraw}
    ...
    \]
    Alternatively, the Krawtchouk polynomials can be implicitly defined as the coefficients of the generating series for $(1+z)^{n-x} (1-z)^x$:
    \[
        (1+z)^{n-x} (1-z)^x=\sum_{k=0}^\infty \kraw{k}{x}{n} z^k.
    \]
\end{definition} %Tim: This is different from the Wiki
...
Using the formalism of Definition~\ref{def:wwdegd}, $f$ is the weightwise quadratic function defined by:% $f_0=0$ and for $k\in [1,n]$ $f_k=x_k + \sum_{i=1}^{n/2} (x_i+1) x_{i+n/2}$.
...
\begin{itemize}
	...
	\item if $n\geq 2$, denoting  $b=(a_1,\cdots,a_{n-2})$ and $y=(x_1,\cdots,x_{n-2})$, we get:
...
\begin{proof}
For $n\geq 2$ we can rewrite $\Dnka{n}{k}{a}$:
