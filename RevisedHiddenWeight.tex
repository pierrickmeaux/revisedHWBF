...
A Boolean function $f\in\BN$ is called balanced if $|\{x \in \F_2^n \mid f(x)=0\}|=|\{x \in \F_2^n \mid f(x)=1\}|=2^{n-1}$. 
...
\begin{proof}
    ...
    Similarly to the proof of Theorem~\ref{theorem:bound_D_nka}, we want to bound the maximum of $|P_{t,a}(z)|$ on $\dis$ for $a\in\F_2^n$, and this will then be an upper bound on $|\Dtkna tkna|$ for every $k\in[0,n]$. 
...
\begin{proof}
	Again, let $q\in\BN$ be given by $q(x)=\sum_{i=1}^{n/2} (x_i+1) x_{i+n/2}$, so that $h=f+q$.
...
%Our approach makes use of generating functions and Cauchy’s estimate to bound the nonlinearity of weightwise quadratic functions from below. 
%This naturally leads to the following question: do the employed techniques, particularly the use of Cauchy’s estimate, extend to different families of Boolean functions? 
%In particular, the same approach seems possible for other weightwise-degree-$d$ functions such that the $f_k$ are direct sums.
%Exploring this could potentially lead to the discovery of new functions with similar computational cost and even better cryptographic properties.
